window.addEventListener('DOMContentLoaded', () => {
        ...
        */
        ...
        <div class="box">
          <div class="box-wrapper">
            <img src="${image}" alt="gambar menu" class="image">
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        if (!value) return menuContainer.innerHTML = '';
    ...
    function renderData({ image, name, price }) {
        ...
        images.setAttribute('alt', 'gambar menu');
    ...
    window.addEventListener('click', event => {
        if (event.target.classList.contains('box-menu')) {
            // tangkap isi dari gambar produk, nama produk dan harga produk
            const item = {
    ...
    btnCheckout.addEventListener('click', function() {
        // jumlah barang
        const menu = Array.from(boxContainer.children).length;
        // jika jumlah barang lebih besar dari angka 0 
        if (menu > 0) {
            // plugin atau library dari "sweetalert2"
            const message = `jumlah barang yang anda beli adalah sebanyak ${menu} barang. dan total biaya yang harus anda keluarkan adalah ${price.textContent}`;
